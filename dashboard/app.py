...
class DataLoadError(Exception):
    """Raised when dashboard data cannot be loaded safely."""


def find_data_file(*filenames: str) -> Path:
    ...
    expected_files = " atau ".join(filenames)
    raise DataLoadError(
        f"File data {expected_files} tidak ditemukan. Pastikan file tersedia di folder data/ repository."
    )
...
def load_and_prepare_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    try:
        day_path = find_data_file("day.csv", "main_data.csv")
        hour_path = find_data_file("hour.csv", "hourly_data.csv")

        day_df = pd.read_csv(day_path)
        hour_df = pd.read_csv(hour_path)
    except (OSError, pd.errors.ParserError) as exc:
        raise DataLoadError(
            "File data tidak bisa dibaca. Pastikan format CSV valid dan file ikut terunggah."
        ) from exc
...
def main() -> None:
    ...
    try:
        ...
        st.error("Aplikasi gagal memuat data. Periksa kembali struktur file dan kolom dataset.")
        st.stop()
